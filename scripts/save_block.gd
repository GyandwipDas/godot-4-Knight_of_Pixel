...
func _on_area_entered(area: Area2D) -> void:
	...
	if area == character.area_2d:
		...
		if game_manager.nodeData["level"] == 1:
			...
			#print(saveblocknum, typeof(saveblocknum))
			#print(memory_count)
			if saveblocknum != 10 && str(game_manager.nodeData["memory"]) != "1" && game_manager.nodeData["level"] != 2:
				print("Mem col off")
		...
		#if saveblocknum == 1 && game_manager.nodeData["level"] == 2:
		if game_manager.nodeData["level"] == 2:
			if saveblocknum == 1:
			#var tween = create_tween()
			#tween.tween_property(croc.audio_stream_player_2d, "volume_db", -80, 15)
				print("Yo")
				croc.hrtbeat_change_to_enabled()
				pass
